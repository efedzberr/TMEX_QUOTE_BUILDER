Replace the ENTIRE contents of `src/lib/kpiTiles.ts` with exactly the code below. Overwrite the whole file — do not merge, do not append, do not add any `export { ... }` statement at the end. The last line of the file must be the closing `}` of `isViewCountable`. Then run `npm run build` and confirm it succeeds.
