...
export function UsersTab({ onToast }: UsersTabProps) {
  ...
  return (
    <div>
      ...
      {/* Invite Modal */}
      {showInvite && <InviteModal onClose={() => setShowInvite(false)} onSuccess={(email) => { onToast(`Invitation sent to ${email}`, 'success'); loadUsers(); }} />}
...
function InviteModal({ onClose, onSuccess }: { onClose: () => void; onSuccess: (email: string) => void }) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setError(null);
    try {
      await callAdminUsers('invite', { email: email.trim(), display_name: displayName.trim(), is_admin: isAdmin });
